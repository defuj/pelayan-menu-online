...
const TransactionDetail = () => {
    ...
    const deleteCart = (id : number) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                setMenu(newMenu);
                calculateTotal(newMenu.filter((item) => item.qty > 0));
            }
        })
    }
    ...
    const removeQtyCart = (id : number) => {
        ...
        setMenu(newCart);
        calculateTotal(newCart.filter((item) => item.qty > 0));
    }
    ...
    const calculateTotal = (newCart : CartModel[] = []) => {
        let newTotal = 0;
        if(newCart.length === 0){
            menu.filter((item) => item.qty > 0).map((item : CartModel) => {
        ...
        return (
            ...
            <div title={slugify(cart.name).toString()} className="cart-items w-100 flex-column my-2" key={cart.name}>
                <div className="d-flex align-items-center flex-row w-100">
                    <div className="flex-column flex-fill">
                        <p className="bodytext1 color-green900 max-line-2 semibold m-0 pb-1">{cart.name}</p>
                        <p className="bodytext2 color-green900 semibold m-0">{cart.qty} Qty</p>
                    </div>
                    <p className="bodytext2 color-green900 semibold m-0">Rp {formatMoney(cart.price*cart.id)}</p>
                </div>
                {cart.note !== '' && <p className="caption m-0 w-100 mt-3" >{cart.note}</p>}
            </div>}
    ...
    useEffect(() => {
        ...
        setTimeout(() => {
            ...
            // merge menu and cart
            menu.forEach((item : ProductModel) => {
                cartData.forEach((cart : CartModel) => {
                    if(item.id === cart.id){
                        item.qty = cart.qty;
                        item.note = cart.note;
                    }
            ...
            setLoading(false);
            setMenu(menu);
            calculateTotal(menu.filter((item) => item.qty > 0));
        }, 2000);
    }, []);
            ...
            <div className="section-product w-100" style={{marginTop: '80px', marginBottom: '120px'}}>
                ...
                <div className="container-customer d-flex flex-column p-3 mx-3 mt-3">
                    ...
                    <div className="input-group-custom d-flex flex-row p-0">
                        <input readOnly={trx.status} onChange={(e) => !trx.status && setTrx({...trx, name: e.target.value})} defaultValue={trx.name} type="text" maxLength={50} name="customerName" id="inputCustomerName" className="bodytext2 form-input" placeholder="Nama Pemesan"/>
                    </div>
                    <p className="bodytext1 color-green900 px-0 m-0 mb-2 mt-2 semibold">
                        Pilih Meja :
                    </p>
                    <div className="input-group-custom d-flex flex-row p-0">
                        <select disabled={trx.status} onChange={(e) => !trx.status && setTrx({...trx, chair: e.target.value})} value={trx.chair} className="custom-select bodytext2 form-input" title="customerChair">
                ...
                <div id="container-product" className="container-product d-flex flex-column px-3">
                    {!loading && menu.filter((item) => item.qty > 0).length > 0 && 
        ...
        <div style={{minHeight: '80px'}} className="container-checkout-cart justify-content-center py-3 px-3 d-flex fixed-bottom bg-white col-xl-4 col-lg-4 col-md-6 col-sm-12 col-12 m-auto flex-column">
            <div className="d-flex flex-column align-items-center w-100">
                ...
                <div className="d-flex w-100">
                    {!trx.accepted && 
                    <button onClick={() => cancelTransaction()} className="mt-2 mr-2 button-cancel w-100 flex-fill bodytext2 semibold text-danger text-center bg-white" type="button">
                        Batalkan Pesanan
                    </button>
                    }
            ...
            <div className="modal-dialog modal-dialog-slideout col-xl-4 col-lg-4 col-md-6 col-sm-12 col-12" role="document">
                <div className="modal-content" style={{maxHeight: `${window.innerHeight-80}px`}}>
                    ...
                    {/* <div className="modal-footer d-flex">
                        <button type="button" className="button-message text-center py-3 flex-fill bodytext2 semibold text-white text-center background-green500" data-dismiss="modal">Tambahkan Menu</button>
                    </div> */}
                </div>
            </div>
        </div>
